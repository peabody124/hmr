...
def batch_rodrigues(theta, name=None):
    ...
    Theta is N x 3
    """
    with tf.compat.v1.name_scope(name, "batch_rodrigues", values=[theta]):
...
def batch_lrotmin(theta, name=None):
    ...
    """
    with tf.compat.v1.name_scope(name, "batch_lrotmin", values=[theta]):
...
def batch_global_rigid_transformation(Rs, Js, parent, rotate_base=False):
    ...
    with tf.compat.v1.name_scope("batch_forward_kinematics", values=[Rs, Js]):
        ...
        def make_A(R, t, name=None):
            ...
            with tf.compat.v1.name_scope(name, "Make_A", values=[R, t]):
