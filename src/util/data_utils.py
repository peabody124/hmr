...
def decode_jpeg(image_buffer, name=None):
    ...
    """
    with tf.compat.v1.name_scope(name, 'decode_jpeg', values=[image_buffer]):
...
def reflect_pose(pose):
    ...
    """
    with tf.compat.v1.name_scope("reflect_pose", values=[pose]):
        """
...
def reflect_joints3d(joints):
    ...
    with tf.compat.v1.name_scope("reflect_joints3d", values=[joints]):
